...
DIP는 이 문제를 해결하기 위해 저수준 모듈이 고수준 모듈에 의존하다록 바꾼다.
...
인프라스트럭처의 기능을 직접 사용하는 것보다 도메인 관점에서 정의한 인터페이스와 실제 구현하기 위한 인터페이스를 추출하여 관리하는게 DIP 설계를 도출하여 시스템을 더 유연하고 테스트하기 쉽게 만들어줄 수 있다.
...
- 단지, 한 패키지에 너무 많은 타입이 몰려서 코드를 차증ㄹ 때 불편한 정도만 아니면 된다.
